...
%  dvips -P pdf  -t landscape go-cheatsheet.dvi
%  ps2pdf go-cheatsheet.ps
...
% 2025-01 (SPB)
% Modified as the basis for a Go cheat sheet.
...
\section{Minimal Go Program}

\texttt{\ldots/helloworld/helloworld.go}:

\lstinputlisting[language=Go]{helloworld.go}
...
To build excutable (to \texttt{helloworld}):
\begin{verbatim}
go build helloworld
\end{verbatim}

\section{Predeclared Types}
\begin{tabular}{l|l|l}
                                   &                                  & \textbf{Zero}  \\
    \textbf{Name}                  & \textbf{Values}                  & \textbf{Value} \\
    \hline
    \texttt{bool}                  & \texttt{true}, \texttt{false}    & \texttt{false} \\
    \texttt{int8}                  & -128 to 127                      & 0              \\
    \texttt{int16}                 & -32768 to 32767                  & 0              \\
    \texttt{int32} / \texttt{rune} & -2147483648 to 2147483647        & 0              \\
    \texttt{int64}                 & -9223372036854775808 to          & 0              \\
                                   & 9223372036854775807              &                \\
    \texttt{int}                   & 32-bit or 64-bit signed          & 0              \\
    \texttt{uint8} / \texttt{byte} & 0 to 255                         & 0              \\
    \texttt{uint16}                & 0 to 65535                       & 0              \\
    \texttt{uint32}                & 0 to 4294967295                  & 0              \\
    \texttt{uint64}                & 0 to 18446744073709551615        & 0              \\
    \texttt{uint}                  & 32-bit or 64-bit unsigned        & 0              \\
    \texttt{uintptr}               & unsigned pointer                 & \texttt{nil}   \\
    \texttt{float32}               & Single precision floating point  & 0              \\
    \texttt{float64}               & Double precision floating point  & 0              \\
    \texttt{complex64}             & 32-bit signed real and imaginary &                \\
    \texttt{complex128}            & 64-bit signed real and imaginary &                \\
    \texttt{string}                & string type (UTF-8 encoded)      & \texttt{""}    \\
\end{tabular}
...
Copyright \copyright\ 2025 Stephen Butler.
\href{https://creativecommons.org/licenses/by-nc-sa/3.0/}{CC BY-NC-SA 3.0}. \\
\href{https://github.com/windscale-stephen/go-cheatsheet}{https://github.com/windscale-stephen/go-cheatsheet} \\
Derived from Winston Chang's LaTeX cheatsheet \href{https://wch.github.io/latexsheet/}{LaTeX cheat sheet}
\end{multicols}
\end{document}
